  ...
  return (
    <View style={{ alignItems: 'center' }}>
      {images.map((image, index) => {
        ...
        return (
          <View style={{zIndex: -index, alignItems: 'center'}} key={`${image}@${index}`}>
          <AnimatedImage
            source={image}
            style={{
              height: size,
          ...
          />
          <AnimatedText onPress={() => onDetails(image, index)} style={collapsed ? {marginTop: -20, zIndex: -index-1} : {zIndex: -index-1}} sharedTransitionTag={`${image}@${index}_text`}>Show details maybe</AnimatedText>
          </View>
        );
      })}
    </View>
...
export function ScreenTwo({ navigation }: StackScreenProps<ParamListBase>) {
  return (
    <ScrollView style={{ flex: 1 }}>
      ...
      <ImageStack
        images={[florence, countryside, dawn, florence, countryside, dawn]}
        onDetails={(image, index) => {navigation.navigate("Screen3", {image, index})}}
      />
    </ScrollView>
  );
}

export function ScreenThree({ navigation, route }: StackScreenProps<ParamListBase>) {
